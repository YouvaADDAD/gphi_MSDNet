...
def get_flops_params(model, input_size):
    ...
    flops.set_op_handle("aten::batch_norm", None)
    #flops.set_op_handle("aten::add", None)
    #flops.set_op_handle("aten::mul", None)
    flops.set_op_handle("aten::sigmoid", None)
    flops.set_op_handle("aten::gelu", None)
    
    #print(flops._ignored_ops)
    flops_total = flops.total() / 1000000.
    #print("FVcore FLOPs(M): ", flops_total)

    params = parameter_count(model)
    params_total = params[""] / 1000000.

    #print("FVcore params(M): ", params_total)

    try:
        num_exit = len(model.classifier)
    except:
        num_exit = 1
    
    counter = flops.by_module()
    ...
    for i in range(num_exit):
        ...
        flop_cpt  += counter["blocks."+str(i)] + counter["classifier."+str(i)]
        param_cpt += params["blocks."+str(i)]  + params["classifier."+str(i)]
        print('---------------------')
        print('FLOPs: %.2fM, Params: %.2fM' % (flop_cpt / 1e6, param_cpt / 1e6))
        flop_per_exit.append(flop_cpt)
        param_per_exit.append(param_cpt)
...
def measure_layer(layer, x):
    ...
    ### ops_nothing
    elif type_name in ['BatchNorm2d', 'Dropout2d', 'DropChannel', 'Dropout', 'DropPath', 'LayerNorm',
                       'MSDNFirstLayer', 'ConvBasic', 'ConvBN', 'InceptionBlock', 'ConvMlp'
                       'ParallelModule', 'MSDNet', 'Sequential', 'InceptionDWConv2d',
