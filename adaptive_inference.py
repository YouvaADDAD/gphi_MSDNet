...
def dynamic_evaluate(model, test_loader, val_loader, args):
    ...
    flops = torch.load(os.path.join(args.log_dir, 'flops.pth'))
    ...
    with open(os.path.join(args.save, 'dynamic.txt'), 'w') as fout:
        for p in range(1, 40):
            ...
            print('valid acc: {:.3f}, test acc: {:.3f}, test flops: {:.2f}M'.format(acc_val, acc_test, exp_flops / 1e7))
            fout.write('({},{}),\n'.format(exp_flops.item() /1e7, acc_test))
